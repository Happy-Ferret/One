...
Section ""
 ...
 #Add registry keys.
 WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${INSTALLATIONNAME}" "DisplayName" "Happy-Ferret's One" 
...
Section "Symbolic links"
 ...
 CreateShortCut "$SMPROGRAMS\${INSTALLATIONNAME}\One.lnk" "$INSTDIR\one.exe" "" "$INSTDIR\one.exe" 0
 CreateShortCut "$DESKTOP\One.lnk" "$INSTDIR\one.exe" "" "$INSTDIR\one.exe" 0 
SectionEnd
